...
#'
#' @details This function can act as a wrapper for the C-based
...
#'   forfurther details.
#' @param t_init numeric; starting temperature for simulated annealing.
#' @param alpha numeric; cooling factor to specify annealing schedule (see
#'   Details). Ignored unless \code{use_optim = FALSE}.
#' @param n integer; the number of annealing iterations, which is also the
#'   number of times the objective function will be evaluated.
#' @param ntemp integer; the number of temperature decreases. Must divide evenly
#'   into the number of annealing iterations (\code{n}). See Details.
#' @param use_optim logical; whether to use \code{\link[stats]{optim}}, which
...
#'
#' @return A vector specifying the state (i.e. values of the decision variables)
...
#' # shortest distance found
#' objective(x_purer)
anneal <- function(x_init, objective, neighbour, t_init, alpha,
                   n = 10000, ntemp = n / 10, use_optim = TRUE) {
  # assertions
  assert_that(is.vector(x_init),
              is.function(objective), is.function(neighbour),
              assertthat::is.number(t_init),
              missing(alpha) || assertthat::is.number(alpha),
              assertthat::is.count(n), assertthat::is.count(ntemp),
              ntemp > 1, ntemp <= n,
              n %% ntemp == 0,
              is.logical(use_optim))

  # number of iterations per temperature decreases
  n_per_temp <- round(n / ntemp)
  if (use_optim) {
    # wrapper for C-based optim function
    x <- optim(x_init, fn = objective, gr = neighbour, method = 'SANN',
               control = list(maxit = n, tmax = n_per_temp, temp = t_init,
                              trace = 0))
    x <- x$par
  } else {
    # custom simulated annealing to emulate Marxana
    assertthat::assert_that(!missing(alpha),
                            alpha > 0, alpha < 1)
    t <- t_init
    x <- x_init
    for (i in 1:n) {
      if (i %% n_per_temp == 0) {
        # decrease temperature; marxan annealing schedule
        t <- alpha * t
      }
      x_try <- neighbour(x)
